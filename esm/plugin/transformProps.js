...
import { getTimeFormatter, smartDateFormatter, getTimeFormatterForGranularity } from '@superset-ui/core';
...
function getDateFormat(props) {
  ...
  const format = columnFormats == null ? void 0 : columnFormats[TIME_COLUMN];
...
function reallyLazyDateRangeFormatFix(dwithf) {
  const datesArr = dwithf.toString().split(" — ");
  const dateObj = new Date(datesArr[0]);
  return dateObj.getMonth() + 1 + "/" + dateObj.getDate();
}

const processDataRecords = (data, dateFormatter, groupby) => {
  if (!data || !data[0]) {
    return data || [];
  }
  /*
  ...
  */
  ...
  let dateHeaders = [];
  let dataRowsObj = {}; // add labels for agg cols in first row
  ...
  for (let x = 0; x < data.length; x++) {
    ...
    const formattedDate = new DateWithFormatter(rawDate, {
    ...
    */
    ...
    for (const key in data[x]) {
      if (key == TIME_COLUMN) continue;
      if (!(key in dataRowsObj)) dataRowsObj[key] = [];
      dataRowsObj[key].push(data[x][key]);
    }
  }

  return [dateHeaders, dataRowsObj];
};
...
export default function transformProps(chartProps) {
  ...
  const data = processDataRecords(queryData.data, dateFormatter, groupby);
